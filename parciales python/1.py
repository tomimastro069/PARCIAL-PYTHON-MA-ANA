...
def solicitar_fechaFactura(factura):
...
def solicitar_codigo(factura):
    while True:
        codigo = input("Ingrese el codigo del producto q desea elegir (0000 para finalizar)")
        if codigo == "0000":
            break
        encontrado = False
...
def imprimir_factura(factura):
    ...
    for i in range(factura["cantidad_articulos"]):
        ...
        print(f"{detalle[0]:<16} {detalle[1]:<16} {detalle[2]:<10} {detalle[3]:<19} {detalle[4]}")
...
def main():
    ...
    solicitar_fechaFactura(factura)
    cuit = solicitar_Cuit(factura)
    solicitar_codigo(factura)
